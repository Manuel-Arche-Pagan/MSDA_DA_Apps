...
# because randomForest() requires the lengths of x and y to be identical, we need to drop rows with any na values. using the following x and y variable assignments solves that problem
mf_bal <- model.frame(formula_lr3, data = train_bal5, na.action = na.omit)
x_bal <- model.matrix(formula_lr3, data = mf_bal)[, -1]
y <- mf_bal$y
...
x_imb <- model.matrix(formula_lr3, data = mf_imb)[, -1]
y <- mf_imb$y
...
test_mm_bal <- model.matrix(formula_lr3, data = mf_test_bal)[,-1]

mf_test_imb <- model.frame(formula_lr3, data = test_imb5, na.action = na.omit)
test_mm_imb <- model.matrix(formula_lr3, data = mf_test_imb)[,-1]

# unlike the previous models, random forest predictions/probabilities can only be appended to the model frames created for the random forest model (because number of rows changed after removing rows with any na values)
...
# above code does not work because one or more variables have zero within-class variance, 
# which lda is unable to process

# variables 'housing' and 'loan' are excluded in the model due to lda's inability
# to train on variables with zero variance within one or more classes
